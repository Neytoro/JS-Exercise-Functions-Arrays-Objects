...
*/
function sortCarInventory(presorted) {

  const sorted = [];
  const models = [];
  const inventory = Object.values(presorted);

  for (var x = 0; x < inventory.length; x++)
    models.push(inventory[x].car_model);

  models.sort();

  for (var y = 0; y < models.length; y++)
    for (var z = 0; z < presorted.length; z++)
      if (models[y] == presorted[z].car_model)
        sorted.push(presorted[z]);
